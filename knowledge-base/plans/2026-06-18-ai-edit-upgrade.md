...
- ✅ **子任务5**：简历导入流式化（P0-4）。后端新增 `POST /api/resume/parse/stream`（SSE，单次 `call_llm_stream` token 进度 + 复用 clean/parse/normalize），与非流式 `/resume/parse`（并行分块）并存、零改动。前端 AIImportModal 全局文本导入走流式、实时显示「已生成 N 字」；分模块与 PDF 路径不变。端到端实测：粘贴文本流式解析成功，结果预览正确（李雷/阿里巴巴/9 项）。提交 `2c8a0b6`。
  > 实现取舍：未改动复杂且并发敏感的并行分块管线，而是新增"单次流式调用"路径专供需要实时进度的导入场景——既得流式 UX 又不冒回归风险。

- ✅ **子任务6**：统一 AI 助手 Dock（P0-1，右下角浮动入口）。把分散的简历 AI 能力收敛到一个常驻可发现面板：简历级「针对 JD 优化」（已填→开弹窗 / 未填→滚动聚焦 JD 区）+ 字段内能力指引（划词改写 / AI 润色 / 语法体检）。**纯增量**：复用既有 JdOptimizeDialog 与逐字段流程，不改动已上线代码，也不与 Agent 对话页 SophiaChat 重复。端到端实测：浮动按钮展开面板、动作与指引正常、暗色适配正确。提交 `556a209`。
  > 实现取舍：完整对话式 Copilot 会与现有 Agent（SophiaChat，本次不可动）重复，故落地为"统一入口 + 能力编排"的 Dock，安全且提升可发现性，不重构可用代码。

> 死代码 `AIPolishDialog.tsx` 按 CLAUDE.md「不相关死代码只指出不删除」保留。移动端按用户「暂时不做」指示整体跳过。

**结论：规划内桌面端 P0/P1 全部落地（P0-1/3/4 + P1-1/2），共 6 个功能子任务，均端到端验证 + 独立提交。**
