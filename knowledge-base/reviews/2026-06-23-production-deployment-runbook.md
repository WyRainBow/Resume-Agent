...
| 本地 `/agent/new` 发消息 `SSE request failed: 401 - 未提供有效的认证信息` | app 的 API/SSE 走**同源**被 Vite 直连后端 9000，**没经 Next 反代、无鉴权注入** | `frontend/.env.local` 补 `VITE_API_VIA_AUTH_WEB=true` → 重启 Vite。该开关让 `getApiBaseUrl()` 指向 Next 反代，会话才能注入 |
| 本地经反代后仍 401 `内部认证信息无效` | 反代注入的 `X-Internal-Auth-Secret` 与后端 env 值**字符串不等**（常见：base64 末尾 `=` 丢失，43 位 vs 44 位） | 对齐 `backend/.env` 与 `web/.env.local` 的 `FASTAPI_INTERNAL_AUTH_SECRET`（`shasum` 比对），改后**重启后端**。注意 kill 旧 uvicorn 要确认端口真释放 |
| 本地用 `127.0.0.1:5173` 登录后一直跳回登录 | `127.0.0.1` 与鉴权层 `localhost` 是不同站点，会话 Cookie 跨站不发送 | 本地统一用 **`http://localhost:5173`** 访问（与 `VITE_AUTH_WEB_URL` 同 host） |
