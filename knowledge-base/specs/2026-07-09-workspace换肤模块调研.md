...
1. **默认皮肤给谁**:新用户默认 Neo(现状)还是默认清新?
2. **切换入口位置**:Workspace 顶栏图标(本文档假设)还是放进「设置」页?
3. **侧边栏(WorkspaceLayout)是否参与换肤**:它是 Workspace/Chat/我的简历等多页共享的外壳,换肤只在 /workspace 路由生效还是整个外壳跟随?(本文档假设:挂在外壳根上,凡用该外壳的页面都跟随)
4. **P4 弹窗是否纳入一期**:弹窗数量大(136 处)但曝光频率低,可以后置。
