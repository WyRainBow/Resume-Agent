...
**当前推进(用户已拍板)**:
1. 预览工具栏"缩放"从底部挪到顶部(纯样式搬家),风险低,可直接做。
2. 预览工具栏"边距开关"+"页数"——按 §3.1 分析,这两个功能天然依赖浏览器端分页渲染(HTML 模板分支),LaTeX 分支(服务端 XeLaTeX 出成品 PDF)做不出真正的"页边距参考线"。实现前需要先确定:是等 `/workspace/html` 接入 Builder 模板(见 §四关联文档)时一起做,还是现在就在 LaTeX 分支上做一个简化版(比如"页数"改成前端用 PDF.js 读取实际页数,"边距"降级为只读展示 `SidePanel` 里已配置的边距值,不做可视化参考线叠加)。**这一步需要和用户确认降级方案是否可接受,再动手实现。**

**暂缓、留档待后续**:
3. 顶部 5 个 Tab 中的求职信 / 联络邮件 / 面试准备 / JD 匹配——用户已确认暂不做,写入本文档留档,后续有需要再单独走 `/superpowers:brainstorming` 立项。
