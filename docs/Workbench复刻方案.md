...
- [x] EditPanel 组件
  ...
  - 工作经验/实习经历编辑（ExperiencePanel）
  - 教育经历编辑（EducationPanel）
  - 专业技能编辑（SkillPanel）
  - **开源经历编辑（OpenSourcePanel）** ✨ 新增
  - **荣誉奖项编辑（AwardPanel）** ✨ 新增
  - 各模块切换显示
- [x] PreviewPanel 组件
  - PDF 预览区域
  - 渲染按钮、下载按钮（loading状态修复）
  - 缩放控制
- [x] TipTap 富文本编辑器（RichEditor）
  - 加粗、斜体、下划线
  - 对齐方式（左/中/右/两端）
  - 无序列表、有序列表（编辑器内显示圆点/数字）
  - 撤销/重做
  - 工具栏 Tooltip 中文提示
  - AI 润色按钮预留
...
| JSON 导入/导出 | ✅ | 🔲 | 待实现 |
| 多语言支持 | ✅ | ❌ | 不需要 |
| LaTeX PDF 渲染 | ❌ (HTML转PDF) | ✅ | **我们独有** |

### 我们的独特优势
1. **真正的 LaTeX 渲染**：使用 xelatex 编译，输出专业级排版 PDF
2. **AI 全局导入**：一键粘贴简历文本，AI 自动解析到各模块
3. **开源经历模块**：针对程序员的独特模块
4. **荣誉奖项模块**：展示获奖经历

### magic-resume 未实现的功能（后续可考虑）
1. **AI 润色**：对段落内容进行 AI 优化
2. **照片上传**：头像显示
3. **模板切换**：多种 LaTeX 模板
4. **JSON 导入/导出**：备份恢复
