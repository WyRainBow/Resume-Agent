...
分析/优化整份简历时，**必须逐模块覆盖**，不允许只挑经历跳过技能：

| 模块 | path | 分析重点 |
|------|------|---------|
| 教育经历 | `education[N].description` | GPA、相关课程、学术成果 |
| 工作/实习经历 | `experience[N].details` | STAR 完整度、量化成果、动词开头 |
| 项目经历 | `projects[N].description` | 技术深度、亮点、量化 |
| **专业技能（重点）** | `skillContent` | **分类清晰度 + AI Native 能力补充 + 无序列表格式** |
| 自我评价 | `selfEvaluation` | 简洁有力、突出核心优势（如有） |

**技能模块（skillContent）专项分析**：
- 判断是否缺少 AI 相关能力（AI 编程工具、LLM API、Prompt 设计、AI Native 思维等）
- 判断分类是否清晰、是否需要重构升级
- 优化建议必须输出 `apply_path: "skillContent"`，`optimized` 为 HTML 无序列表

**整份优化输出要求**：每个有内容的模块都要输出对应的 optimization_suggestion，至少覆盖经历 + 项目 + 技能三类。
