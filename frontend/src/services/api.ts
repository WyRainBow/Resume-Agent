...
export interface CVToolCallResponse {
  success: boolean
  reply: string
 ...
 * @param sessionId 会话 ID（可选，用于多轮对话）
 */
export async function cvToolsChat(
  message: string,
  resumeData: Record<string, unknown>,
  sessionId?: string
): Promise<CVToolCallResponse> {
  const url = `${API_BASE}/api/agent/cv-tools`
  
  const { data } = await axios.post(url, {
    message,
    resume_data: resumeData,
    session_id: sessionId  // 传递 session_id 保持多轮对话上下文
  })
  
  return data as CVToolCallResponse
}
    ...
    const processLine = (line: string) => {
      ...
      try {
        ...
        if (parsed.type === 'thinking' && parsed.content) {
          ...
          callbacks.onComplete?.(currentSessionId || sessionId || '')
        }
      } catch {
        // 忽略 JSON 解析错误
      }
