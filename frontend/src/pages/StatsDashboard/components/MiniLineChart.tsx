  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_10px_26px_rgba(15,23,42,0.06)]">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-lg font-bold tracking-tight text-slate-900">近7天投递趋势</h3>
        <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600">application_date</span>
      </div>
      {data.length === 0 ? (
        <div className="flex h-[280px] items-center justify-center rounded-xl bg-slate-50 text-slate-500">暂无趋势数据</div>
      ) : (
        <svg viewBox={`0 0 ${width} ${height}`} className="h-[280px] w-full">
          ...
          {points.map((p, idx) => (
            ...
            >
              <circle cx={p.x} cy={p.y} r="4.2" fill="#FFFFFF" stroke="#4F7CFF" strokeWidth="2" />
            </motion.g>
          ))}
