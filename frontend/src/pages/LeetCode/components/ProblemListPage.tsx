    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <div className="mx-auto max-w-[1440px] px-6 py-8">
        ...
        <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_24px_64px_rgba(148,163,184,0.18)]">
          <div className="grid grid-cols-[1.2fr_120px_1fr_160px_120px] border-b border-slate-200 px-5 py-4 text-xs uppercase tracking-[0.2em] text-slate-400">
            <div>标题</div>
            <div>难度</div>
            <div>标签</div>
            <div>更新时间</div>
          ...
          {!loading && !error && filtered.map(problem => (
            <div key={problem.slug} className="grid grid-cols-[1.2fr_120px_1fr_160px_120px] items-center border-b border-slate-100 px-5 py-4 text-sm transition hover:bg-slate-50">
              <div>
              ...
              <div className="text-xs text-slate-400">{new Date(problem.updatedAt).toLocaleString()}</div>
