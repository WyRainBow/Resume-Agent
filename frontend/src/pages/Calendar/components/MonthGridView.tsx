  ...
  return (
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="flex border-b border-slate-200 bg-white">
        {/* 左侧占位，对齐周视图的时间轴列 */}
        <div className="w-20 shrink-0 border-r border-transparent" />
        <div className="grid flex-1 grid-cols-7 bg-slate-50/80">
          {WEEK_LABELS.map((label) => (
            <div key={label.en} className="px-3 py-2 border-l border-slate-200 first:border-l-0">
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{label.en}</div>
            </div>
          ))}
        </div>
      </div>
